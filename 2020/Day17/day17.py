    ...
    for x_new in range(x-1, x+2):
        for y_new in range(y-1, y+2):
            for z_new in range(z-1, z+2):
                neigh_coords.append((x_new, y_new, z_new))
...
for i in range(6):
    new_active_coords = run_cycle(min_x, max_x, min_y, max_y, min_z, max_z, curr_active_coords)          
